...
async function enviarStatusBot(status) {
  ...
  try {
    ...
    if (response.ok) console.log("Mensagem enviada com sucesso!");
  } catch (error) {
    console.error("Erro ao enviar mensagem:", error);
  }
}

// ROTA DA WEB: É aqui que o UptimeRobot vai acessar para manter acordado!
app.get("/", (req, res) => {
...
app.listen(PORT, () => {
  ...
  // Assim que o servidor liga, ele avisa no Discord que está ON
  enviarStatusBot("ON");
});